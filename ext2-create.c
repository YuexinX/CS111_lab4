...
void write_block_bitmap(int fd)
{
	...
	for (u32 i = 0; i < LAST_BLOCK; i++){
		u32 row = i/8;
		u32 col = i%8;
		map_value[row] = map_value[row] | (1 << col);
	}
...
void write_inode_table(int fd) {
	...
	memset(hello_inode.i_block, 0, sizeof(hello_inode.i_block)); // Ensure i_block is clean
	memcpy(hello_inode.i_block, "hello-world", strlen("hello-world") + 1); // Copy the string into i_block
...
void write_hello_world_file_block(int fd)
{
	...
	if (write(fd, hello_world, strlen(hello_world)) != 12){
		errno_exit("write");
	}
}
